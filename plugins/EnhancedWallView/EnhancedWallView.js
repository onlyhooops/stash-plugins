...
(function() {
  ...
  // 仅匹配 /images 或 /scenes 根列表页，不匹配 /tags、/galleries 及详情页
  const isListPath = (path) => /^\/(images|scenes)\/?$/.test(path || window.location.pathname);

  // ==================== 抢先隐藏原始内容（脚本解析时立即执行）====================
  (function hideOriginalImmediately() {
    if (isListPath() && document.documentElement) {
  ...
  const DEFAULT_CONFIG = {
    columnWidth: 260,        // 列宽 (px)
    columnGap: 6,            // 列间距 (px)
    rowGap: 6,               // 行间距 (px)
    itemsPerPage: 40,        // 每页加载数量
    loadThreshold: 600,      // 距离底部多少像素时开始加载
    videoPreviewDelay: 300,  // 鼠标悬停多久后开始预览视频 (ms)
    enableLightbox: true,    // 点击在内置 lightbox 中浏览，而非进入详情页
    enableOnImages: true,
    enableOnScenes: true,
    debug: false,            // 调试模式
  };
  ...
  function getPageType() {
    ...
    // 排除详情页面 (如 /images/123)
    if (/\/(images|scenes)\/\d+/.test(path)) {
      log('这是详情页面，跳过');
      return null;
    }
    
    const cfg = getConfig();
    if (path.includes('/images') && cfg.enableOnImages) return 'images';
    if (path.includes('/scenes') && cfg.enableOnScenes) return 'scenes';
    return null;
  }
  ...
  class EnhancedWall {
    ...
    async addItemsBatch(itemsData) {
      const cfg = getConfig();
      const colWidth = cfg.columnWidth;
          ...
          <div class="enhanced-wall-settings-body">
            <div class="enhanced-wall-settings-section">
              ...
              <div class="enhanced-wall-settings-row">
                <label>列宽 (px)</label>
                <input type="number" id="ew-setting-columnWidth" min="120" max="600" step="10" value="${cfg.columnWidth}">
              </div>
              <div class="enhanced-wall-settings-row">
                <label>列间距 (px)</label>
                <input type="number" id="ew-setting-columnGap" min="0" max="48" step="2" value="${cfg.columnGap}">
              </div>
              <div class="enhanced-wall-settings-row">
                <label>行间距 (px)</label>
                <input type="number" id="ew-setting-rowGap" min="0" max="48" step="2" value="${cfg.rowGap}">
              </div>
            </div>
            <div class="enhanced-wall-settings-section">
              <h4>加载与滚动</h4>
              ...
              <div class="enhanced-wall-settings-row">
                <label>无限滚动触发距离 (px)</label>
                <input type="number" id="ew-setting-loadThreshold" min="200" max="2000" step="100" value="${cfg.loadThreshold}">
              </div>
            </div>
            <div class="enhanced-wall-settings-section">
              <h4>预览</h4>
              <div class="enhanced-wall-settings-row enhanced-wall-settings-checkbox">
                <label><input type="checkbox" id="ew-setting-enableLightbox" ${cfg.enableLightbox !== false ? 'checked' : ''}> 点击在内置 lightbox 中浏览（不跳转详情页）</label>
              </div>
              <div class="enhanced-wall-settings-row">
                <label>视频悬停预览延迟 (ms)</label>
                <input type="number" id="ew-setting-videoPreviewDelay" min="0" max="2000" step="100" value="${cfg.videoPreviewDelay}">
              </div>
            </div>
            <div class="enhanced-wall-settings-section">
              <h4>启用页面</h4>
              <div class="enhanced-wall-settings-row enhanced-wall-settings-checkbox">
                <label><input type="checkbox" id="ew-setting-enableOnImages" ${cfg.enableOnImages ? 'checked' : ''}> 图片</label>
              </div>
              <div class="enhanced-wall-settings-row enhanced-wall-settings-checkbox">
                <label><input type="checkbox" id="ew-setting-enableOnScenes" ${cfg.enableOnScenes ? 'checked' : ''}> 短片</label>
              </div>
            </div>
            <div class="enhanced-wall-settings-section">
              <h4>其他</h4>
              <div class="enhanced-wall-settings-row enhanced-wall-settings-checkbox">
                <label><input type="checkbox" id="ew-setting-debug" ${cfg.debug ? 'checked' : ''}> 调试模式</label>
              </div>
            </div>
          </div>
          <div class="enhanced-wall-settings-footer">
    ...
    populateSettingsForm(modal) {
      const cfg = getConfig();
      const ids = ['columnWidth', 'columnGap', 'rowGap', 'itemsPerPage', 'loadThreshold', 'videoPreviewDelay'];
      ids.forEach(id => {
        const el = modal.querySelector(`#ew-setting-${id}`);
        if (el) el.value = cfg[id];
      });
      const checkIds = ['enableOnImages', 'enableOnScenes', 'enableLightbox', 'debug'];
      checkIds.forEach(id => {
        const el = modal.querySelector(`#ew-setting-${id}`);
    ...
    saveSettings() {
      const updates = {
        columnWidth: parseInt(document.getElementById('ew-setting-columnWidth')?.value, 10) || DEFAULT_CONFIG.columnWidth,
        columnGap: parseInt(document.getElementById('ew-setting-columnGap')?.value, 10) || DEFAULT_CONFIG.columnGap,
        rowGap: parseInt(document.getElementById('ew-setting-rowGap')?.value, 10) || DEFAULT_CONFIG.rowGap,
        itemsPerPage: parseInt(document.getElementById('ew-setting-itemsPerPage')?.value, 10) || DEFAULT_CONFIG.itemsPerPage,
        loadThreshold: parseInt(document.getElementById('ew-setting-loadThreshold')?.value, 10) || DEFAULT_CONFIG.loadThreshold,
        videoPreviewDelay: parseInt(document.getElementById('ew-setting-videoPreviewDelay')?.value, 10) ?? DEFAULT_CONFIG.videoPreviewDelay,
