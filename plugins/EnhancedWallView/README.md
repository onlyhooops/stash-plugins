...
1. 仅替换 `/images`、`/scenes` 列表页的墙/网格内容，不隐藏原生筛选条
2. 使用 `PluginApi.Event` 监听 `stash:location` 实现路由感知
...
|----|------|
| 列宽 / 列间距 / 行间距 | 瀑布流布局参数 |
| 每页加载数量 | 每次请求条数（建议 24–60） |
| 无限滚动触发距离 | 距底部像素开始加载 |
| 启用 lightbox | 点击图片在内置 lightbox 浏览，否则跳转详情页 |
| 视频悬停预览延迟 | 悬停多久后开始预览（ms） |
| 启用页面 | 图片 / 短片 开关 |
| 调试模式 | 控制台输出日志 |
